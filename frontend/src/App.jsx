                ...
                <div className="h-full flex flex-col items-center justify-center text-gray-400 gap-3 py-10">
                  {isSubmitting ? (
                    <>
                      ...
                      <span className="text-sm">正在准备输出内容...</span>
                    </>
                  ) : (
                    <>
                      <FileCode2 size={32} />
                      <span className="text-sm">启动审查后，AI 实时分析报告将在此显示</span>
                    </>
                  )}
                </div>
              )}
      ...
      <main className="w-full max-w-6xl">
        ...
        {activeTab === 'review' ? (
          <div className="glass-panel p-8 md:p-10">
            ...
            {/* 审查报告区域 */}
            {(showReportPanel || isFullscreenMode) && renderReportContent(isFullscreenMode)}
          </div>
        ) : (
